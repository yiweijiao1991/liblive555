...
unsigned fileSinkBufferSize = 2*100000; //yzhu: change 100000 into 2*100000,to adapt to Amba 720p encoding or bigger
		...
		else 
		{
			...
			while ((subsession = iter.next()) != NULL) 
			{
					...
					(strcmp(subsession->codecName(), "H264") == 0)) 
				{
					...
					fileSink = MyH264VideoFileSink::createNew(*env, outFileName,
						fileSinkBufferSize,
						oneFilePerFrame);
				} 
				else 
...
public:
    qosMeasurementRecord(struct timeval const& startTime, RTPSource* src)
		...
		{ // hack for RealMedia sessions (RDT, not RTP)
...
#ifdef SUPPORT_REAL_RTSP
		if (session->isRealNetworksRDT)
			src = (RTPSource*)(subsession->readSource()); // hack
#endif
	...
	*env << "stream_availability\t" << (statusCode == 0 ? 100 : 0) << "\n";
	...
	if (session != NULL) 
	{
		...
		while ((subsession = iter.next()) != NULL) 
		{
			RTPSource* src = subsession->rtpSource();
#ifdef SUPPORT_REAL_RTSP
			if (session->isRealNetworksRDT) src =
				(RTPSource*)(subsession->readSource()); // hack
#endif
			if (src == NULL) continue;
...
void checkForPacketArrival(void* /*clientData*/) 
{
	...
	arrivalCheckTimerTask
		...
		(TaskFunc*)checkForPacketArrival,
		NULL);
}
