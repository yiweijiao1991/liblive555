...
//  printf("fOurMaxPacketSize %d\n", fOurMaxPacketSize);	//jay
}
...
//	printf("seqNo %d\n", fSeqNo);		//jay
	...
	u_int16_t cur_seqno = fSeqNo;

	if (cur_seqno < 10) {	// skip rtcp packet
...
//	printf("packFrame - haveOverflowData\n");	//jay
...
//		printf("packFrame - read new fragment\n");	//jay
		// Normal case: we need to read a new frame from the source
	  if (fSource == NULL) return;

	  fCurFrameSpecificHeaderPosition = fOutBuf->curPacketSize();
	  fCurFrameSpecificHeaderSize = frameSpecificHeaderSize();	// normal case 0
	  fOutBuf->skipBytes(fCurFrameSpecificHeaderSize);
	  fTotalFrameSpecificHeaderSizes += fCurFrameSpecificHeaderSize;	// normal case 0	

//	printf("  packFrame: getNextFrame\n");	//jay
	  fSource->getNextFrame(fOutBuf->curPtr(), fOutBuf->totalBytesAvailable(),
			  afterGettingFrame, this, ourHandleClosure, this);
	}
//	printf("  packFrame-end\n");	//jay
}
...
//	printf("						  MultiFramedRTPSink::afterGettingFrame\n");		//jay
...
::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
				 struct timeval presentationTime,
				 unsigned durationInMicroseconds) {
//	printf("							  MultiFramedRTPSink::afterGettingFrame1\n");	//jay
	if (fIsFirstPacket) {
...
#if 0		//optm
	else 		//schedule delayed task for next send
	{
		...
		//	  printf("timeNow %d sec %d usec\n", timeNow.tv_sec, timeNow.tv_usec);	//jay
		int uSecondsToGo;
		...
		//	  printf("				scheduleDelayedTask: this %p, sendNext after %d us\n", &envir().taskScheduler(), uSecondsToGo);	//jay
...
//	printf("sendNext...\n");	//jay
	...
	sink->buildAndSendPacket();
//	printf("sendNext done\n");	//jay
}
