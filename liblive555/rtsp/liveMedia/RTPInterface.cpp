...
RTPInterface::RTPInterface(Medium* owner, Groupsock* gs)
	: fOwner(owner), fGS(gs),
	  ...
	  fNeedFifo(False), fExitFlag(False) {
...
//	makeSocketNonBlocking(fGS->socketNum());		// will cause EWOULDBLOCK errno when sendto packet
	increaseSendBufferTo(envir(), fGS->socketNum(), 50*1024);	// default to 50*1024
//	printf("send buffer %d\n", getSendBufferSize(envir(), fGS->socketNum()));	//jay
}
...
void RTPInterface::createFifo(void)
{
	fNeedFifo = 1;
	create_fifo(1000000, 64);
	pthread_create(&fTid, NULL, doSendPacketThread, this);
	printf("@@doSendPacketThread created, arg %p\n", this);
}
...
void* RTPInterface::doSendPacketThread(void* arg)
{
	u_int8_t *packet;
	u_int32_t packetSize;
	RTPInterface *rpt_interface = (RTPInterface *)arg;

	while(!rpt_interface->fExitFlag) {
//		printf("%d packets available...\n", check_packet_num());
		read_one_packet(&packet, &packetSize);
		rpt_interface->doSendPacket(packet, packetSize);
//		sleep(1);
	}
	printf("@@doSendPacketThread exiting...\n");
	return NULL;
}

void RTPInterface::doSendPacket(unsigned char* packet, unsigned packetSize) {
//	printf("doSendPacket: this %p, addr %p, size %d\n", this, packet, packetSize);	//jay
...
#if 0
	static u_int16_t pre_seqno;
	u_int16_t cur_seqno = (ntohl(*(u_int32_t *)packet)) & 0xffff;

	if (cur_seqno != 6) {	// skip rtcp packet
		if (cur_seqno != pre_seqno + 1)
			printf("seq %d -> %d\n", pre_seqno, cur_seqno);
		pre_seqno = cur_seqno;
	}
#endif
