...
//	printf("RTSPServer::removeServerMediaSession\n");	//jay
...
::RTSPClientSession(RTSPServer& ourServer, unsigned sessionId,
	...
	resetRequestBuffer();
//	printf("RTSPClientSession: turnOnBackgroundReadHandling\n");	//jay
  ...
  if (!parseRTSPRequestString((char*)fRequestBuffer, fRequestBytesAlreadySeen,
	  ...
	  if (strcmp(cmdName, "OPTIONS") == 0) {
		...
		else if (strcmp(cmdName, "SETUP") == 0) 
		{
			if(standardFlag != 1)
			{
				char const*tempRequestBuffer = "DESCRIBE rtsp://192.168.2.88:554/stream1 RTSP/1.0\r\nCSeq: 2\r\nUser-Agent: Web Client\r\nUser-Type: 4\r\nAccept: application/sdp\r\n\r\n";
				handleCmd_DESCRIBE("2", urlSuffix, (char const*)tempRequestBuffer);
...
static void parseTransportHeader(char const* buf,
	...
	while (sscanf(fields, "%[^;]", field) == 1) {
		...
		} else if (_strncasecmp(field, "multicast", 9) == 0) {
			isMulticast = True;
...
#define RTSP_ALLOW_CLIENT_DESTINATION_SETTING
