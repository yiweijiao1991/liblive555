...
MyJPEGVideoRTPSink
::MyJPEGVideoRTPSink(UsageEnvironment& env, Groupsock* RTPgs)
  : VideoRTPSink(env, RTPgs, 26, 90000, "JPEG") {
}

MyJPEGVideoRTPSink::~MyJPEGVideoRTPSink() {
}

MyJPEGVideoRTPSink*
MyJPEGVideoRTPSink::createNew(UsageEnvironment& env, Groupsock* RTPgs) {
  return new MyJPEGVideoRTPSink(env, RTPgs);
}

Boolean MyJPEGVideoRTPSink::sourceIsCompatibleWithUs(MediaSource& source) {
  return source.isJPEGVideoSource();
}

Boolean MyJPEGVideoRTPSink
...
::doSpecialFrameHandling(unsigned /*fragmentationOffset*/,
  ...
  MyJPEGVideoSource* source = (MyJPEGVideoSource*)fSource;
  
  if (source == NULL) return; // sanity check

  u_int8_t mainJPEGHeader[8]; // the special header
  u_int32_t fragmentationOffset = source->fragmentOffset();
...
unsigned MyJPEGVideoRTPSink::specialHeaderSize() const {
  // Our source is known to be a JPEGVideoSource
  MyJPEGVideoSource* source = (MyJPEGVideoSource*)fSource;
...
Boolean MyJPEGVideoRTPSink::continuePlaying() {
	 MyJPEGVideoSource* source = (MyJPEGVideoSource*)fSource;
	 source->setMaxOutputPacketSize(ourMaxPacketSize() - 20); //RTP hdr size (12) + min jpeg head size (8)
